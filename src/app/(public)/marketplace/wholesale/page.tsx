...
const icons = [Boxes, ReceiptText, Truck];

export default function WholesalePage() {
  const { dict } = useLanguage();
  const { wholesalePage } = dict;
    ...
    <div className="min-h-screen bg-[#faf9f6]">
      <div className="max-w-screen-xl mx-auto px-6 lg:px-12 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-16">
          {wholesalePage.steps.map(({ title, body }, i) => {
            const Icon = icons[i];
            return (
              <div
                key={title}
        ...
        <div className="bg-[#1e3d18] rounded-3xl px-8 py-10 sm:px-14 sm:py-14 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="max-w-lg">
            <p
              ...
              style={{ fontFamily: "Georgia, serif" }}
            >
              {wholesalePage.ctaTitle}
            </p>
            <p className="text-[14px] text-white/70 leading-relaxed">
              {wholesalePage.ctaDescription}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            >
              {wholesalePage.emailWholesaleTeam}
            </a>
            <Link
              href="/support"
